...
void color_pixel(int x, int y) {
	if (pixels_current[POSITION(x, y)] == .125) {
		pixels_next[POSITION(x, y)] = 1;
		pixels_next[POSITION(x, y) + 1] = 0;
		pixels_next[POSITION(x, y) + 2] = 1;
	}
	else if (pixels_current[POSITION(x, y)] == .25) {
		pixels_next[POSITION(x, y)] = 1;
		pixels_next[POSITION(x, y) + 1] = 0;
		pixels_next[POSITION(x, y) + 2] = 0;
	}
	else if (pixels_current[POSITION(x, y)] > .375) {
		pixels_next[POSITION(x, y)] = 1;
	...
	else if (pixels_current[POSITION(x, y)] > .625) {
		pixels_next[POSITION(x, y)] = 0;
		pixels_next[POSITION(x, y) + 1] = 1;
		pixels_next[POSITION(x, y) + 2] = 1;
	}
	else if (pixels_current[POSITION(x, y)] > .75) {
		pixels_next[POSITION(x, y)] = 0;
		pixels_next[POSITION(x, y) + 1] = .5;
		pixels_next[POSITION(x, y) + 2] = 1
	}
	else if (pixels_current[POSITION(x, y)] > .875) {
		pixels_next[POSITION(x, y)] = 0;
		pixels_next[POSITION(x, y) + 1] = 0;
		pixels_next[POSITION(x, y) + 2] = 1;
	}
...
void avalanch(struct pixel_position temp) {
	...
	//Sand goes left
	if ((x + 1)<window_width) {
		pixels_current[POSITION(x + 1, y)] += .125;
		color_pixel(x+1,y);
	}
	...
	if ((y + 1)<window_height) {
		pixels_current[POSITION(x, y + 1)] += .125;		
		color_pixel(x, y+1);
	}
	...
	pixels_current[POSITION(x-1, y - 1)] += .125;
	color_pixel(x-1, y - 1);
...
void drawPile()
{
	...
	/*while (cc < 10000) {*/
		...
		// 1 pile
		pixels_current[POSITION(window_width / 2, window_height / 2)] += .125;
	...
	/*	int xLower = window_width / 4 + 100;
		...
		if ((double)avalanchCount / (double)total_Sand > 1){
